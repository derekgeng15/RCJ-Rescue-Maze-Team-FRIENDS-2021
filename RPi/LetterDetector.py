...
def cuts(img, direction, height, width, value = 0, bf=False):
    ...
    otherModifier = 38 # Modifier for BOTTOM
    ...
    img[height-TBCUT-otherModifier:height, :] = value # Cut BOTTOM of image
...
def getLetter(img, direction="right", showFrame=True, frameCounting=False, frameCount=1): #if we want to export imgs
    ...
    thresh = cv2.threshold(blurred, 85, 255, cv2.THRESH_BINARY)[1] # 73 for easy 2
    if showFrame:
    ...
    if gbr=="S" or gbr==None: # ADDED FOR DIFFICULT 2 | or gbr==None:
        ...
        thresh = cuts(thresh, direction, height, width, 255, bf=True) # BF TRUE only for DIFFICULT 2
        #print(thresh)
...
def processLetter(thresh, showFrame=True, frameCounting=False, frameCount=1, bfr=True):
    ...
    areaFiltered = areaFilter(thresh, maxArea = 400) # Easy field 2 - 400
    ...
    for i, c in enumerate(contours):
        if(cv2.contourArea(c)>500 and cv2.contourArea(c) < 4500): #550, 10000 for easy field 2
            ...
            if cv2.contourArea(c) > 4500 or cv2.contourArea(c) < 550: # 550, 9000 for easy field 2
                continue
            ...
            if len(contop) == 2 and len(conmid) == 1 and len(conbot) == 2:
                ...
                areaFilter(left, maxArea=30)
                areaFilter(mid2, maxArea=30)
                areaFilter(right, maxArea=30)
